...
resource "aws_iam_policy" "ssm_policy" {
  name        = "ssp-product-service-ssm-policy-${var.environment}"
  description = "Allows reading specific SSM parameters for the product service"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow",
        Resource = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.mongo_uri_param_name}"
      }
    ]
  })
}
...
module "ecs_service" {
  ...
  environment_variables = [
    { name = "MONGO_URI_PARAM_NAME", value = var.mongo_uri_param_name },
